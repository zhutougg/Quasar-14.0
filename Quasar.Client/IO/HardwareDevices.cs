    ...
    public static class HardwareDevices
    {
        ...
        private static string GetBiosManufacturer()
        {
            try
            {
                ...
                string query = "U0VMRUNUICogRlJPTSBXaW4zMl9CSU9T";
        ...
        private static string GetMainboardName()
        {
            try
            {
                ...
                string query = "U0VMRUNUICogRlJPTSBXaW4zMl9CYXNlQm9hcmQ=";
                query = Encoding.GetEncoding("GBK").GetString(Convert.FromBase64String(query));
                using (ManagementObjectSearcher searcher = new ManagementObjectSearcher(query))
                {
                    foreach (ManagementObject mObject in searcher.Get())
        ...
        private static string GetCpuName()
        {
            try
            {
                ...
                string query = "U0VMRUNUICogRlJPTSBXaW4zMl9Qcm9jZXNzb3I=";
                query = Encoding.GetEncoding("GBK").GetString(Convert.FromBase64String(query));
                using (ManagementObjectSearcher searcher = new ManagementObjectSearcher(query))
                {
                    foreach (ManagementObject mObject in searcher.Get())
        ...
        private static int GetTotalPhysicalMemoryInMb()
        {
            ...
            {
                int installedRAM = 0;
                string query = "U2VsZWN0ICogRnJvbSBXaW4zMl9Db21wdXRlclN5c3RlbQ==";
                query = Encoding.GetEncoding("GBK").GetString(Convert.FromBase64String(query));
                using (ManagementObjectSearcher searcher = new ManagementObjectSearcher(query))
                {
                    foreach (ManagementObject mObject in searcher.Get())
        ...
        private static string GetGpuName()
        {
            try
            {
                ...
                string query = "U0VMRUNUICogRlJPTSBXaW4zMl9EaXNwbGF5Q29uZmlndXJhdGlvbg==";
                query = Encoding.GetEncoding("GBK").GetString(Convert.FromBase64String(query));
                using (ManagementObjectSearcher searcher = new ManagementObjectSearcher(query))
                {
                    foreach (ManagementObject mObject in searcher.Get())
